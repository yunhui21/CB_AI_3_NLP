# Day_11_02_RnnBasic_4_different.py
...
def rnn_basic_4_different(words, loop_count):
    ...
    for i in range(loop_count):
        ...
        # print([''.join(vocab[arg[:limited]]) for arg, limited in zip(preds_arg, len_array)])
        print([''.join(vocab[arg[:cnt]]) for arg, cnt in zip(preds_arg, len_array)])

    sess.close()


rnn_basic_4_different(['tensor', 'sky', 'bird'], loop_count=100)
