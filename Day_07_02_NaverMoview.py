...
# 문제 2
# 김윤 박사 cnn sentence를 검색해서 github 검색
...
# 문제 4
# 모든 문서의 토큰 갯수를 그래프로 그래보세요.

# 문제 5
# 모든 문서의 토큰 길이를 25개로 맞춰 주세요.
# (25개를 넘어서는 만큼은 삭제하고, 부족한 부분은 공백 토믄으로 재웁니다.)
...
def make_same_length(documents, max_len):
    ...
    for docs in documents:
        ...
        if doc_len < max_len:
            new_docs.append(docs + [''] * (max_len - doc_len))
        else:
            new_docs.append(docs[:max_len])
...
train_set = make_feature_data(x_train, y_train, vocab) # make_feature_data 함수
test_set = make_feature_data(x_test, y_test, vocab)

clf = nltk.NaiveBayesClassifier.train(train_set)
print('acc :', nltk.classify.accuracy(clf, test_set)) # acc : 0.76166
